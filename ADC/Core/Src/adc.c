...
void ADCx_Init(ADC_TypeDef *ADCx, uint8_t Channel)
{
	...
	if(Channel == ADC_Channel_0)
		GPIO_Init(GPIOA, 0, INPUT_ANALOG, NOPULL, 0);
	if(Channel == ADC_Channel_1)
		GPIO_Init(GPIOA, 1, INPUT_ANALOG, NOPULL, 0);
	if(Channel == ADC_Channel_2)
		GPIO_Init(GPIOA, 2, INPUT_ANALOG, NOPULL, 0);
	if(Channel == ADC_Channel_3)
		GPIO_Init(GPIOA, 3, INPUT_ANALOG, NOPULL, 0);
	if(Channel == ADC_Channel_4)
		GPIO_Init(GPIOA, 4, INPUT_ANALOG, NOPULL, 0);
	if(Channel == ADC_Channel_5)
		GPIO_Init(GPIOA, 5, INPUT_ANALOG, NOPULL, 0);
	if(Channel == ADC_Channel_6)
		GPIO_Init(GPIOA, 6, INPUT_ANALOG, NOPULL, 0);
	if(Channel == ADC_Channel_7)
		GPIO_Init(GPIOA, 7, INPUT_ANALOG, NOPULL, 0);
